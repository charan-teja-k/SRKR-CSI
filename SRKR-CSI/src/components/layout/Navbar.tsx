...
const Navbar: React.FC = () => {
  ...
  const headerClasses = `no-scrollbar fixed top-0 left-0 right-0 z-50 transition-all duration-300 w-full border-b overflow-hidden ${
    isOpen
  ...
  return (
    ...
    <nav className={headerClasses}>
          ...
          <div className="hidden md:flex items-center space-x-8">
            {navLinks.map((link) => (
              <Link
                key={link.path}
                to={link.path}
                className={`text-sm font-medium transition-all duration-300 relative pt -2 ${
                  location.pathname === link.path
                    ? 'text-[#2d6cea]' // Active: Blue
                    : 'text-[#d4defa] hover:text-[#002a94]' // Default: Light Blue, Hover: Blue
                }`}
              >
                {link.label}
                {/* Active Indicator Dot (White Underline/Dot) */}
                {location.pathname === link.path && (
                  <span className="absolute -bottom-0.5 left-0 w-full h-0.5 bg-[#2d6cea] rounded-full shadow-sm -mt-0.5" />
                )}
              </Link>
            ))}
      ...
      <div
        className={`absolute top-full left-0 w-full bg-[#030817] border-t border-[#d4defa]/10 shadow-2xl md:hidden flex flex-col transition-all duration-300 ease-in-out overflow-hidden ${
          isOpen ? 'max-h-screen opacity-100' : 'max-h-0 opacity-0'
        }`}
      >
        <div className="px-4 py-6 space-y-3 h-screen bg-[#030817]">
          {navLinks.map((link) => (
            <Link
              key={link.path}
              to={link.path}
